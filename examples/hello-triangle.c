...
createSurfaceDependent(void)
{
    ...
    depthImage     = obdn_CreateImage(
            oMemory, dim.width, dim.height, depthFormat,
            VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_IMAGE_ASPECT_DEPTH_BIT,
            VK_SAMPLE_COUNT_1_BIT, 1, OBDN_MEMORY_DEVICE_TYPE);
    VkImageView attachments_0[2] = {obdn_GetSwapchainImageView(swapchain, 0),
                                    depthImage.view};
    VkImageView attachments_1[2] = {obdn_GetSwapchainImageView(swapchain, 1),
                                    depthImage.view};
    obdn_CreateFramebuffer(device, 2, attachments_0, dim.width, dim.height,
                           renderPass, &framebuffers[0]);
    obdn_CreateFramebuffer(device, 2, attachments_1, dim.width, dim.height,
...
initApp(void)
{
    ...
    if (WIREFRAME)
        obdn_CreateGraphicsPipeline_Taurus(device, renderPass, pipelineLayout,
                                           VK_POLYGON_MODE_LINE, &pipeline);
    else
        obdn_CreateGraphicsPipeline_Taurus(device, renderPass, pipelineLayout,
                                           VK_POLYGON_MODE_FILL, &pipeline);
...
draw(i64 fi, i64 dt)
{
    ...
    const Obdn_Frame* fb = obdn_AcquireSwapchainFrame(swapchain, VK_NULL_HANDLE,
                                                      acquireSemaphore);
    ...
    obdn_CmdBeginRenderPass_ColorDepth(cmd.buffer, renderPass,
